...
import { getAllCollections, getCollection, getSpriteIdentifierFromMode, findSpriteByIdentifier } from "@/constants/spriteCollections";
import { 
  Select, 
  SelectContent, 
  SelectItem, 
...
export function SpriteControls({
  ...
  // Check if any selected sprite is a line sprite
  const hasLineSprite = useMemo(() => {
    if (!spriteState.selectedSprites || spriteState.selectedSprites.length === 0) {
      return false;
    }
    return spriteState.selectedSprites.some(identifier => {
      // Check if it's a shape-based line sprite
      if (identifier === "shape:line") {
        return true;
      }
      // Check if it's a line sprite by finding the sprite and checking its id
      const spriteInfo = findSpriteByIdentifier(identifier);
      return spriteInfo?.sprite.id === "line";
    });
  }, [spriteState.selectedSprites]);
